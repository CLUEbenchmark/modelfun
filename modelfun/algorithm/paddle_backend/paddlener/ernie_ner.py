...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="clue",
        type=str,
        choices=["msra_ner", 'clue'],
        help="The named entity recognition datasets.")
    parser.add_argument(
        "--output_dir",
        default="./tmp/best_msra_ner_model",
        type=str,
...
def load_custom_dataset(data_path):
        ...
        with open(ds, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                for entity in each['entities']:
                    if 'B-' + entity['label'] not in rng:
                        rng.append('B-' + entity['label'])
                        rng.append('I-' + entity['label'])
                    labels[entity['start_offset']] = 'B-' + entity['label']
                    for i in range(entity['start_offset']+1, entity['end_offset']):
                        labels[i] = 'I-' + entity['label']
                # print({'tokens': [i for i in text], 'label': labels})
                yield {'tokens': [i for i in text], 'ner_tags': labels}
        # # 跳过列名
    ...
    return {'train': map_ds, 'val': map_ds, 'test': map_ds}

@paddle.no_grad()
def evaluate(model, loss_fct, metric, data_loader, mode="valid"):
    model.eval()
    metric.reset()
...
def run(args):
    ...
    try:
        ...
        raw_datasets = load_custom_dataset('datasets/cluener/valdoc.json')
        label_list = raw_datasets['train'].labels
