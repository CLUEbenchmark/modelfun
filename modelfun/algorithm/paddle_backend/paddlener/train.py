...
parser.add_argument("--dataset", default="msra_ner", type=str, choices=["msra_ner", "peoples_daily_ner", "weibo"] ,help="The named entity recognition datasets.")
...
"""
python -u ./train.py     --model_type ernie     --model_name_or_path ernie-3.0-base-zh     --dataset weibo     --max_seq_length 128     --batch_size 32     --learning_rate 2e-1     --num_train_epochs 3     --logging_steps 1     --save_steps 500     --output_dir ./tmp/msra_ner/     --device gpu
"""
@paddle.no_grad()
...
def do_train(args):
    ...
    if args.dataset in ["peoples_daily_ner", 'msra_ner']:
        raw_datasets = load_dataset(args.dataset)
    else:
        # read_weibo, data_path=
        raw_datasets = load_dataset('./datasets/ner_ds/weibo')
    ...
    print(len(train_ds))
    train_ds = train_ds.select(range(int(0.01*len(train_ds))))
