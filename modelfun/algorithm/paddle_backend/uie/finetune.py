...
def do_train():
    ...
    loss_list = []
    global_step = 0
    total_global_step = len(train_ds) * args.num_epochs // args.batch_size
    print('total steps', total_global_step)
    best_step = 0
    best_f1 = 0
    tic_train = time.time()
    patience = 0
    for epoch in range(1, args.num_epochs + 1):
        for batch in train_data_loader:
            ...
            if (global_step % (total_global_step//10) == 0 and rank == 0):
            # if epoch % args.valid_steps == 0 and rank == 0:
