...
from model import ErnieForPretraining, ErnieMLMCriterion, RobertaForPretraining, BertForPretraining
...
parser.add_argument("--p_embedding_num", type=int, default=3, help="number of p-embedding")
...
parser.add_argument("--rdrop_coef", default=0, type=float, help="The coefficient of KL-Divergence loss in R-Drop paper, for more detail please refer to https://arxiv.org/abs/2106.14448), if rdrop_coef > 0 then R-Drop works")
...
def do_train():
    ...
    # pretrain_model = 'ernie-3.0-base-zh'  
    pretrain_model = 'macbert-base-chinese'
    # pretrain_model = 'ernie-1.0'
    if pretrain_model.startswith('ernie'):
        model = ErnieForPretraining.from_pretrained(pretrain_model)
        tokenizer = ppnlp.transformers.ErnieTokenizer.from_pretrained(pretrain_model)
    else:
        # model = RobertaForPretraining.from_pretrained(pretrain_model)
        model = BertForPretraining.from_pretrained(pretrain_model)
        tokenizer = ppnlp.transformers.AutoTokenizer.from_pretrained(pretrain_model)
