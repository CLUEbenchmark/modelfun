...
def train_ner(train_path: str, test_path: str, unlabeled_path: str,
    ...
    model = AutoModelForTokenClassification.from_pretrained(backbone, num_labels=2*len(schemas)+1)

    def model_init():
        return AutoModelForTokenClassification.from_pretrained(backbone, num_labels=2*len(schemas)+1)

    # TODO train ner model.
    ...
    label_names = [f"{schema}_B" for schema in schemas] + [f"{schema}_I" for schema in schemas] + ["O"]
    ...
    # TODO check why we need substract 1 from index
