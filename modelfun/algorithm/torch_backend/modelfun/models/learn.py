...
def main(lr=0.005, maml_lr=0.01, iterations=1000, ways=5, shots=1, tps=32, fas=5, device=torch.device("cpu"),
         download_location="./cached_datasets"):
    # dataset = l2l.text.datasets.NewsClassification(root=download_location, download=True)
    
    from task_datasets.eprstmt_dataset import RawTextDataset
    
    train_dataset = RawTextDataset("cached_datasets/eprstmt/train_0.json")
    val_dataset = RawTextDataset("cached_datasets/eprstmt/dev_0.json")
    train_dataset = l2l.data.MetaDataset(train_dataset)
    val_dataset = l2l.data.MetaDataset(val_dataset)

    classes = list(range(2)) # 41 classes
    # classes = 
    ...
    train_gen = l2l.data.TaskDataset(
            train_dataset, num_tasks=2, 
            task_transforms=[
                l2l.data.transforms.FusedNWaysKShots(
                    # train_dataset, n=ways, k=shots, filter_labels=classes[:20]),
                    train_dataset, n=ways, k=shots, filter_labels=None),
                l2l.data.transforms.LoadData(train_dataset),
                l2l.data.transforms.RemapLabels(train_dataset)],)

    validation_gen = l2l.data.TaskDataset(
            validation_dataset, num_tasks=2, 
            task_transforms=[
                l2l.data.transforms.FusedNWaysKShots(
                    validation_dataset, n=ways, k=shots, filter_labels=None),
    ...
    test_gen = l2l.data.TaskDataset(
            test_dataset, num_tasks=2, 
            task_transforms=[
                l2l.data.transforms.FusedNWaysKShots(
                    test_dataset, n=ways, k=shots, filter_labels=None),
...
def maml_fewshot_cls(train_path: Union[str, List], unlabeled_path: str, oss: bool=True):
    ...
    if isinstance(unlabeled_path, str):
        ...
        unlabeled_text = train_path
    print('in few shot')
    model_name = 'lr'
    if model_name == 'maml':  # TODO add MAML here
        model = None
        res = model.predict_proba(unlabeled_text)
        pred_labels = np.argmax(res, axis=-1)
        final_predictions = []
        for idx, value in enumerate(pred_labels):
            if res[idx, value] < 0.5:  # TODO  set confidence threshold.
